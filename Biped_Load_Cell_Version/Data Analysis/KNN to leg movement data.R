...
simple_zmp_locations_exp_1.data <- as.data.frame(matrix(0, ncol = 5, nrow = (nrow(zmp_locations_exp_1.data)/25)))
simple_zmp_row <-1

x = replicate(nrow(zmp_locations_exp_1.data), 0)

for (i in 1:nrow(zmp_locations_exp_1.data)) {         # Going through all the rows
  ...
  if(distance == (12)){                   #To pick only one line of data recorded per position
    for (j in 1:ncol(zmp_locations_exp_1.data)){
      simple_zmp_locations_exp_1.data[simple_zmp_row,j] <- zmp_locations_exp_1.data[i,j]
    }
...
save(simple_zmp_locations_exp_1.data,file="simple_zmp_locations_exp_1.Rda")
